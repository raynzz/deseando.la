...
export default function WishDetail() {
  ...
  useEffect(() => {
    ...
    const fetchExtras = async () => {
      ...
      try {
        ...
        const clean = baseUrl.replace(/\/+$/, '');
        // Trae eventos del deseo
        const evRes = await fetch(
          `${clean}/items/events?fields=id,title,date,location,description,wish&filter[wish][_eq]=${encodeURIComponent(
            id
          )}&sort=date`
        );
        // Trae gifts del deseo
        const giRes = await fetch(
          `${clean}/items/gifts?fields=id,title,price,status,description,wish,image&filter[wish][_eq]=${encodeURIComponent(
            id
          )}&sort=title`
        );
  ...
  const ownerAvatar = useMemo(
    () => assetUrl(baseUrl, (wish as any)?.owner?.avatar),
    [baseUrl, wish]
  );

  // People relacionados (si el wish ya trae un array `people`)
  const relatedPeople: PersonT[] = useMemo(() => {
    const raw = (wish as any)?.people;
    if (!raw) return [];
    // soporta estructuras tipo [{person:{...}}] o [{...}] según tu rel M2M
    return (Array.isArray(raw) ? raw : [])
      .map((p: any) => (p?.person ? p.person : p))
      .filter(Boolean);
  }, [wish]);
  ...
  return (
    <main className="container mx-auto max-w-4xl p-6 space-y-6">
      ...
      {!isLoading && !isError && wish && (
        <article className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          ...
          <div className="p-5 md:p-6">
            <div className="text-sm opacity-70">ID: {(wish as any).id}</div>
            <h2 className="text-2xl font-semibold">{safeText((wish as any).title, '(sin título)')}</h2>

            <div className="mt-3 grid gap-3 text-sm md:grid-cols-3">
              <div>
                <span className="font-semibold">Visibilidad:</span> {(wish as any).visibility}
              </div>
              <div>
                <span className="font-semibold">Estado:</span> {(wish as any).status}
              </div>
      ...
      {/* Personas relacionadas (si vienen en el wish) */}
      {!isLoading && !isError && wish && (
        <section className="space-y-3">
          <h3 className="text-lg font-semibold">Personas</h3>
          {relatedPeople.length === 0 ? (
            <p className="opacity-70">Sin personas relacionadas.</p>
          ) : (
            <ul className="grid gap-4 sm:grid-cols-2">
              {relatedPeople.map((p, idx) => {
                const full = [p.first_name, p.last_name].filter(Boolean).join(' ') || 'Sin nombre';
                const avatar = assetUrl(baseUrl, p.avatar || null);
                return (
                  <li key={`${p.id}-${idx}`} className="flex items-center gap-3 rounded-2xl border p-4 shadow-sm">
                    {avatar ? (
                      <img src={avatar} alt={full} className="h-12 w-12 rounded-full object-cover" />
                    ) : (
                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-neutral-200 text-sm">
                        🙂
                      </div>
                    )}
                    <div>
                      ...
                      <div className="text-sm opacity-70">
                        {safeText(p.title || p.role, 'Miembro de la red')}
                      </div>
                    </div>
                  </li>
                );
                  ...
                  <div className="p-4">
                    ...
                    {g.price != null && (
                      <div className="text-sm opacity-80">
                        Precio: ${Number(g.price).toFixed(2)}
                      </div>
                    )}
            ...
            {events.map((ev) => (
              <li key={ev.id} className="rounded-2xl border p-4 shadow-sm">
                ...
                <div className="mt-1 text-sm opacity-80">
                  {ev.date ? new Date(ev.date).toLocaleString() : 'Sin fecha'}
                  {ev.location ? ` · ${ev.location}` : ''}
                </div>
                {ev.description && (
                  <p className="mt-1 text-sm opacity-80">{ev.description}</p>
                )}
              </li>
            ))}
          </ul>
